...
export default function GroupChat() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, async (snapshot) => {
      ...
      const missingUids = docs.map((d) => d.sender).filter((uid) => !userMap[uid]);
      const uniqueMissing = [...new Set(missingUids)];

      if (uniqueMissing.length > 0) {
        const userDocs = await Promise.all(
          uniqueMissing.map((uid) => getDoc(doc(db, "users", uid)))
        );
        const newMap: { [key: string]: string } = {};
        userDocs.forEach((docSnap, i) => {
          const uid = uniqueMissing[i];
          newMap[uid] = docSnap.exists() ? docSnap.data().name : "Unknown";
        });
  ...
  const uploadMedia = async (uri: string) => {
    try {
      ...
      const storageRef = ref(storage, `groupMedia/${groupId}/${Date.now()}`);
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <TouchableOpacity
          ...
          onPress={() => router.push({ pathname: "/groupDetails", params: { groupId } })}
        >
          <MaterialIcons name="info-outline" size={24} color="#1E90FF" />
        </TouchableOpacity>
      </View>
      ...
      <FlatList
        ...
        renderItem={({ item, index }) => {
          ...
          const showName = !isMyMessage;
          const time = moment(item.timestamp?.toDate()).format("HH:mm");
          const messageDate = moment(item.timestamp?.toDate()).format("DD/MM/YYYY");
          const prevDate = index > 0 ? moment(messages[index - 1].timestamp?.toDate()).format("DD/MM/YYYY") : null;

          return (
            <>
              {messageDate !== prevDate && <Text style={styles.dateHeader}>{messageDate}</Text>}

              {!isMyMessage && showName && (
                <Text style={styles.senderName}>{userMap[item.sender] || "Loading..."}</Text>
              )}
              ...
              >
                <View style={isMyMessage ? styles.myMessageContainer : styles.otherMessageContainer}>
                  {item.mediaUrl ? (
                    ...
                    <Text style={isMyMessage ? styles.myMessage : styles.otherMessage}>{item.text}</Text>
                  )}

                  <View style={styles.messageInfo}>
                    {isMyMessage && (
                      <Text style={[styles.statusText, styles.sentText]}>
                        Seen by {item.seenBy?.filter((uid) => uid !== auth.currentUser?.uid).length || 0}
                      </Text>
                    )}
                    <Text style={isMyMessage ? styles.timestamp : styles.timestampOther}>{time}</Text>
                  </View>
                </View>
              </TouchableOpacity>
            </>
